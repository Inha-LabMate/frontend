...
export default function PortfolioSection({ data }: Props) {const { t } = useTranslation();
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : t("PDF 텍스트 추출에 실패했습니다.");
      alert(`❌ ${errorMessage}`);
      if (fileInputRef.current) {
        fileInputRef.current.value = "";
      }
  ...
  const handleNotionLinkExtract = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : t("노션 텍스트 추출에 실패했습니다.");
      alert(t("❌ {{errorMessage}}\\n\\n💡 노션 페이지가 공개 설정되어 있는지 확인해주세요.", { errorMessage: errorMessage }));
    } finally {
      setIsExtracting(false);
    }
  };
  ...
  return (
    <>
      <Section
        ...
        {data.length === 0 ?
        <div className="p-6 text-center text-black">{t("등록된 포트폴리오가 없습니다.")}
        ...
        <table className="w-full">
            <thead className="bg-gray-100">
              <tr>
                <th className="px-4 py-2 text-center text-sm font-medium text-black border">{t("포트폴리오명")}

              </th>
                <th className="px-4 py-2 text-center text-sm font-medium text-black border">{t("내용 미리보기")}

              </th>
                <th className="px-4 py-2 text-center text-sm font-medium text-black border">{t("파일/링크")}

              </th>
                <th className="px-4 py-2 text-center text-sm font-medium text-black border">{t("소스 타입")}
            ...
            <tr key={portfolio.id} className="hover:bg-gray-50">
                ...
                <a
                  ...
                  className="text-inha-blue hover:underline inline-flex items-center gap-1">
                        {portfolio.fileUrl.includes("notion") ? "🔗" : "📄"}{t("보기")}
                </a> :
                ...
                <span className="text-gray-400">-</span>
                  ...
                  "bg-blue-100 text-blue-700"}`
                      ...
                      {portfolio.fileUrl?.includes("notion") ? t("노션") :
      ...
      <Modal
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            <label className="w-32 text-sm text-black">
              <span className="text-red-500">*</span>{t("포트폴리오명")}
            </label>
            <input
              type="text"
              value={formData.type}
          ...
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 bg-gray-50">
            <div className="flex items-center justify-between mb-3">
              <div className="text-sm font-medium text-black">{t("포트폴리오 추가 방법 선택")}

              </div>
              <div className="text-xs text-gray-500">{t("자동으로 텍스트가 추출됩니다")}
            ...
            <div className="mb-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <label htmlFor="pdf-upload" className="text-sm font-medium text-black">{t("📄 PDF 파일 업로드")}
              ...
              <div className="ml-6 flex items-center gap-2">
                  ...
                  className="px-4 py-2 bg-inha-blue text-white rounded hover:bg-blue-700 transition-colors whitespace-nowrap cursor-pointer text-sm font-medium">{t("파일 선택")}
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <label htmlFor="notion-link" className="text-sm font-medium text-black">{t("🔗 노션 페이지 링크")}

                </label>
                <span className="text-xs text-gray-500">{t("(공개된 페이지만 가능)")}</span>
              </div>
              ...
              <div className="ml-6 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    className="px-4 py-2 bg-inha-blue text-white rounded hover:bg-blue-700 transition-colors whitespace-nowrap text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed">
                      {isExtracting ? t("추출 중...") : t("텍스트 추출")}
                    </button>
                  </div>
                  <div className="text-xs text-gray-500 bg-blue-50 p-2 rounded">{t("💡 노션 페이지 공유 방법: 페이지 우측 상단 \"공유\" → \"웹에 게시\" 클릭 후 링크 복사")}
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-center gap-3">
              ...
              <span className="text-sm text-black">
                {formData.sourceType === "pdf" ? t("PDF에서 텍스트를 추출하고 있습니다...") : t("노션 페이지에서 텍스트를 추출하고 있습니다...")
          ...
          <div className="flex items-start gap-4">
            <label className="w-32 text-sm pt-2 text-black">
              <span className="text-red-500">*</span>{t("포트폴리오 내용")}
            </label>
            <div className="flex-1">
              <textarea
                ...
                placeholder={t("PDF 업로드 또는 노션 링크에서 자동으로 추출되거나, 직접 입력할 수 있습니다.")} />

              <div className="text-xs text-gray-500 mt-1">
                {formData.content.length}{t("/ 5000자")}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-center gap-2 mt-6">
          <button
            ...
            className="px-6 py-2 bg-inha-blue text-white rounded hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">{t("확인")}
          ...
          <button
            ...
            className="px-6 py-2 bg-gray-500 text-white rounded hover:bg-gray-600 transition-colors">{t("닫기")}
